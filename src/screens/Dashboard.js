...
export default function Dashboard({ navigation }) {
    ...
    const _getSongs = useCallback(() => {
        setFetching(true)
        setLoading(true)
        APIService.getAllSongs().then(data => {
            setFetching(false)
            setLoading(false)
            if (data) {
                setSongs(data)
    ...
    const _onSongItemPressed = useCallback(trackId => {
    ...
    const _renderSongItem = useCallback(({ item }) => {
        const { trackName, artworkUrl60, artistName, trackTimeMillis, trackId } = item
        return <SongCard
            trackId={trackId}
            trackName={trackName}
