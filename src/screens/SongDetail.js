...
const _renderTopView = ({ trackName, artworkUrl, collectionName, artistName }) => {
...
const SongDetail = ({ route }) => {
    const { trackName,
        ...
        trackTimeMillis,
        primaryGenreName,
        trackCount } = route.params?.track || {}
    return (
        <ScrollView contentContainerStyle={styles.container} >
            {_renderTopView({ trackName, artworkUrl, artistName, collectionName })}
            <View style={styles.section}>
                <Header title="GENRE" subtitle={primaryGenreName} />
                <Header title="PLAYBACK TIME" subtitle={`${Math.round(TimeUtil.convertMillisToMinute(trackTimeMillis))}m`} />
            </View>
            <View style={styles.section}>
                <Header title="COLLECTION PRICE" subtitle={`${collectionPrice}  ${currency}`} />
...
const styles = StyleSheet.create({
    container: { paddingVertical: 14, paddingHorizontal: 5, flexDirection: "column" },
    section: {
        flexDirection: "row",
        marginTop: 20,
